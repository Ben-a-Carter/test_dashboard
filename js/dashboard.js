...
function pTheme(){
  ...
  return{
    bg:d?'#1a1e24':'#fff',
    text:d?'#edf1f5':'#3d3d3a',
    grid:d?'#30363d':'#e2e0db'
  };
}
...
function themedLayout(l={}){
  ...
  return{
    ...
    paper_bgcolor:t.bg,
    plot_bgcolor:t.bg,
    font:{color:t.text,...(l.font||{})},
    xaxis:{gridcolor:t.grid,zerolinecolor:t.grid,...(l.xaxis||{})},
    yaxis:{gridcolor:t.grid,zerolinecolor:t.grid,...(l.yaxis||{})}
  };
}

function initTheme(){
  themeState.mode='system';
  localStorage.removeItem('analytics-theme-mode');
  ...
  const apply=()=>{
    ...
    updateThemeButton();
    if(activeData.length)renderDashboard();
  };
  ...
  media.addEventListener('change',()=>{
    if(themeState.mode==='system')apply();
  });
...
function updateThemeButton(){
  ...
  b.setAttribute(
    'aria-label',
    `Change appearance. Current setting: ${l.textContent}`
  );

  b.title=
    themeState.mode==='system'
      ?'System. Click to switch to Light.'
      :`${l.textContent}. Click to switch to ${themeState.mode==='light'?'Dark':'Light'}.`;
}
...
function resetState(){
  ...
  [
    ...
    'distribution',
    'flow'
  ].forEach(k=>plotFilters[k]={});

  tableState.sortField='id';
  tableState.sortDirection='asc';
}

function applyDropdowns(d){
  return d.filter(r=>
    Object.entries(dropdownState).every(
      ([f,v])=>v==='all'||String(r[f])===String(v)
    )
  );
}

function applyPlots(d,exclude=null){
  return d.filter(r=>{
    for(const[source,filters]of Object.entries(plotFilters)){
      if(source===exclude)continue;

      for(const[f,v]of Object.entries(filters)){
        if(f==='ids'){
          if(!v.includes(r.id))return false;
        }else if(String(r[f])!==String(v)){
          return false;
        }
      }
    }
...
function dataFor(source=null){
  return applyPlots(
    applyDropdowns(activeData),
    source
  );
}
...
function initializeFilterAnchorControl(){
  ...
  const label=document.getElementById('anchorFiltersLabel');
  ...
  function applyAnchorState(){
    ...
    if(anchored){
      label.textContent='Filters anchored';
      button.title='Allow filters to move with the page';
    }else{
      label.textContent='Anchor filters';
      button.title='Keep filters visible while scrolling';
    }
  }
...
function buildUI(){
  ...
  c.filters.forEach(([f,label])=>{
    ...
    a.value='all';
    a.textContent=`All ${label.toLowerCase()}`;

    s.appendChild(a);

    uniq(activeData,f).forEach(v=>{
    ...
    s.addEventListener('change',e=>{
      dropdownState[f]=e.target.value;

      if(e.target.value==='all'){
        removeFilterFromRecency('dropdown',f);
      }else{
        markFilterAsMostRecent('dropdown',f);
      }
  ...
  [
    ['trend','trendTitle','trendHelp'],
    ['category','categoryTitle','categoryHelp'],
    ['status','statusTitle','statusHelp'],
    ['scatter','scatterTitle','scatterHelp'],
    ['heatmap','heatmapTitle','heatmapHelp'],
    ['distribution','distributionTitle','distributionHelp'],
    ['flow','flowTitle','flowHelp']
  ].forEach(([k,t,h])=>{
    document.getElementById(t).textContent=c[k].title;
    document.getElementById(h).textContent=c[k].help;
  });
...
function renderMetrics(){
  ...
  configs[activeTab].metrics.forEach(([l,fn])=>{
    ...
    c.innerHTML=
      ...
      `<div class="metric-value">${fn(filteredData)}</div>`;
...
function renderActive(){
  ...
  Object.entries(dropdownState).forEach(([f,v])=>{
    if(v!=='all'){
      chip(`${f}: ${v}`);
    }
  });

  Object.entries(plotFilters).forEach(([src,fs])=>
    Object.entries(fs).forEach(([f,v])=>
      chip(
        `${src}: ${
          f==='ids'
            ?`${v.length} selected`
  ...
  if(!n){
    ...
    s.className='no-filter';
    s.textContent='No filters applied';

    c.appendChild(s);
  }
}

function formatFilterPart(field,value){
  if(field==='ids'){
...
function getFiltersAffectingPlot(targetSource){
  const filterMap=new Map();

  Object.entries(dropdownState).forEach(([field,value])=>{
    if(value==='all'){
      return;
    }
    ...
    const filter={
      kind:'dropdown',
      source:field,
      label:formatFilterPart(field,value)
    };
  ...
  Object.entries(plotFilters).forEach(([source,sourceFilters])=>{
...
function clearAppliedFilter(filter){
  if(filter.kind==='dropdown'){
    dropdownState[filter.source]='all';
...
function renderPlotFilterChips(){
  document
    .querySelectorAll('[data-filter-chips]')
    .forEach(container=>{
      const targetSource=
...
function renderResets(){
  renderPlotFilterChips();
}

function renderTrend(){
  const c=configs[activeTab].trend;
  const d=dataFor('trend');
  const xs=uniq(d,c.x);

  const bars=xs.map(x=>{
    const s=d.filter(
      r=>String(r[c.x])===String(x)
    );
  ...
  Plotly.react(
    ...
    themedLayout({
      ...
      yaxis:{
        title:c.valueField||'Count'
      },
      yaxis2:{
        overlaying:'y',
        side:'right',
        ticksuffix:
          c.lineField&&!c.lineNumeric
            ?'%'
            :''
      },
      legend:{
...
function renderCategory(){
  const c=configs[activeTab].category;
  const d=dataFor('category');
  const cats=uniq(d,c.field);
  const stacks=uniq(d,c.stack);

  const tr=stacks.map((sv,i)=>({
    x:cats,
    y:cats.map(cat=>{
      const s=d.filter(
        r=>
          String(r[c.field])===String(cat)&&
          String(r[c.stack])===String(sv)
      );
...
function renderStatus(){
  const c=configs[activeTab].status;
  const d=dataFor('status');
  const labels=uniq(d,c.field);

  const values=labels.map(v=>
    d.filter(
      r=>String(r[c.field])===String(v)
    ).length
  );
...
function renderScatter(){
  const c=configs[activeTab].scatter;
  const d=dataFor('scatter');
  const groups=uniq(d,c.color);

  const tr=groups.map((g,i)=>{
    const rows=d.filter(
      r=>String(r[c.color])===String(g)
    );
  ...
  Plotly.react(
    ...
    p.on('plotly_selected',e=>{
      if(!e?.points?.length){
        return;
      }

      markFilterAsMostRecent(
        'plot',
        'scatter'
      );
...
function renderHeatmap(){
  const c=configs[activeTab].heatmap;
  const d=dataFor('heatmap');

  const rows=uniq(d,c.row);
  const cols=uniq(d,c.col);

  const z=rows.map(rv=>
    cols.map(cv=>{
      const s=d.filter(
        r=>
          String(r[c.row])===String(rv)&&
          String(r[c.col])===String(cv)
      );
...
function renderDistribution(){
  const c=configs[activeTab].distribution;
  const d=dataFor('distribution');
...
function renderFlow(){
  const c=configs[activeTab].flow;
  const d=dataFor('flow');

  const a=uniq(d,c.first);
  const b=uniq(d,c.second);
  const cc=uniq(d,c.third);
  ...
  a.forEach((x,i)=>
    b.forEach((y,j)=>{
      const n=d.filter(
        r=>
          String(r[c.first])===String(x)&&
          String(r[c.second])===String(y)
      ).length;

      if(n){
        source.push(i);
        target.push(a.length+j);
        value.push(n);
      }
    })
  );
  ...
  b.forEach((x,i)=>
    cc.forEach((y,j)=>{
      const n=d.filter(
        r=>
          String(r[c.second])===String(x)&&
          String(r[c.third])===String(y)
      ).length;

      if(n){
        source.push(a.length+i);
        target.push(
          a.length+
          b.length+
          j
...
function exportPlotImage(
  ...
  return exportPlotImageWithTitle(
    plot,
    format
  );
}
...
function applyPlotHoverOutline(
  plot,
  point
){
  if(!plot||!point){
...
function clearPlotHoverOutline(plot){
  if(!plot||!plot.data){
    return;
  }
...
function attachHoverOutline(plotId){
  const plot=
    document.getElementById(plotId);
  ...
  plot.on(
    'plotly_unhover',
    ()=>{
      clearPlotHoverOutline(plot);
    }
  );
}

function getPlotExportTitle(plot){
  const card=
    plot.closest('.chart-card');
...
function initializeExportControls(){
  ...
  Object.entries(plots).forEach(
    ([plotId,source])=>{
      ...
      if(
        !plot||
        plot.parentElement.querySelector(
          `.chart-export-control[data-export-source="${source}"]`
        )
      ){
        return;
      }
      ...
      control.className=
        'chart-export-control';

      control.dataset.exportSource=
        source;
      ...
      button.innerHTML=
        '<span class="chart-export-icon" aria-hidden="true">⇩</span><span>Export</span>';
...
function attachAllHoverOutlines(){
  [
    'trendChart',
    'categoryChart',
    'statusChart',
...
function attachCat(
  ...
  p.on('plotly_unhover',()=>{
    document
      ...
      .textContent=
        'Hover over a categorical mark to highlight related records. Click it to filter the other views.';
...
function clearListeners(p){
  if(!p?.removeAllListeners){
    return;
  }
...
function renderTable(){
  ...
  document
    ...
    .textContent=
      `${shown.length.toLocaleString()} shown of ${filteredData.length.toLocaleString()}`;
}
...
function formatVal(f,v){
  ...
  if(
    /salary|budget|actual|amount|capacity|aid_offer|financial_need/.test(f)
  ){
    return `$${Math.round(
      Number(v)
    ).toLocaleString()}`;
