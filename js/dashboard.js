...
let activeTab='student',activeData=[],filteredData=[];
const dropdownState={},plotFilters={},tableState={sortField:null,sortDirection:'asc'},filterRecency=[];

function filterRecencyKey(kind,source){return `${kind}:${source}`;}
function markFilterAsMostRecent(kind,source){const key=filterRecencyKey(kind,source),i=filterRecency.indexOf(key);if(i!==-1)filterRecency.splice(i,1);filterRecency.unshift(key);}
function removeFilterFromRecency(kind,source){const key=filterRecencyKey(kind,source),i=filterRecency.indexOf(key);if(i!==-1)filterRecency.splice(i,1);}
function clearFilterRecency(){filterRecency.length=0;}

const colors={navy:'#1f3864',blue:'#2e75b6',green:'#548235',orange:'#d98c31',red:'#c94c4c',purple:'#7665a8',gray:'#73726c'};
const plotConfig={responsive:true,displaylogo:false,displayModeBar:false};

function rngFactory(seed){return function(){let t=seed+=0x6D2B79F5;t=Math.imul(t^t>>>15,t|1);t^=t+Math.imul(t^t>>>7,t|61);return((t^t>>>14)>>>0)/4294967296;};}
function normal(r,m=0,s=1){let u=0,v=0;while(!u)u=r();while(!v)v=r();return m+s*Math.sqrt(-2*Math.log(u))*Math.cos(2*Math.PI*v);}
function pick(r,a){return a[Math.floor(r()*a.length)];}
function clamp(x,a,b){return Math.max(a,Math.min(b,x));}
function logit(x){return 1/(1+Math.exp(-x));}
function round(x,d=1){const f=10**d;return Math.round(x*f)/f;}
function sum(d,f){return d.reduce((s,r)=>s+Number(r[f]||0),0);}
function avgRaw(d,f){return d.length?sum(d,f)/d.length:0;}
function avg(d,f,p=1){return avgRaw(d,f).toFixed(p);}
function pct(d,f,v){return d.length?`${(d.filter(r=>String(r[f])===String(v)).length/d.length*100).toFixed(1)}%`:'-';}
function currency(v){return Math.abs(v)>=1e6?`$${(v/1e6).toFixed(1)}M`:Math.abs(v)>=1e3?`$${(v/1e3).toFixed(1)}K`:`$${Math.round(v).toLocaleString()}`;}
function uniq(d,f){return[...new Set(d.map(r=>r[f]).filter(v=>v!==null&&v!==undefined&&v!==''))].sort((a,b)=>typeof a==='number'&&typeof b==='number'?a-b:String(a).localeCompare(String(b)));}
...
function resolvedTheme(){if(themeState.mode==='light'||themeState.mode==='dark')return themeState.mode;return window.matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light';}
function pTheme(){const d=getTheme()==='dark';return{bg:d?'#1a1e24':'#fff',text:d?'#edf1f5':'#3d3d3a',grid:d?'#30363d':'#e2e0db'};}
function themedLayout(l={}){const t=pTheme();return{...l,paper_bgcolor:t.bg,plot_bgcolor:t.bg,font:{color:t.text,...(l.font||{})},xaxis:{gridcolor:t.grid,zerolinecolor:t.grid,...(l.xaxis||{})},yaxis:{gridcolor:t.grid,zerolinecolor:t.grid,...(l.yaxis||{})}};}

function initTheme(){
  themeState.mode='system';
  const media=window.matchMedia('(prefers-color-scheme: dark)');
  const apply=()=>{document.documentElement.setAttribute('data-theme',resolvedTheme());updateThemeButton();if(activeData.length)renderDashboard();};
  document.getElementById('themeToggle').addEventListener('click',()=>{themeState.mode=themeState.mode==='system'?'light':themeState.mode==='light'?'dark':'light';apply();});
  media.addEventListener('change',()=>{if(themeState.mode==='system')apply();});
  apply();
}
function updateThemeButton(){
  const b=document.getElementById('themeToggle'),i=document.getElementById('themeIcon'),l=document.getElementById('themeLabel');
  if(themeState.mode==='system'){i.textContent='\u25D0';l.textContent='System';}
  else if(themeState.mode==='light'){i.textContent='\u2600';l.textContent='Light';}
  else{i.textContent='\u263D';l.textContent='Dark';}
  b.setAttribute('aria-label',`Change appearance. Current setting: ${l.textContent}`);
  b.title=themeState.mode==='system'?'System. Click to switch to Light.':`${l.textContent}. Click to switch to ${themeState.mode==='light'?'Dark':'Light'}.`;
}

function resetState(){
  clearFilterRecency();
  Object.keys(dropdownState).forEach(k=>delete dropdownState[k]);
  ['trend','category','status','scatter','heatmap','distribution','flow'].forEach(k=>plotFilters[k]={});
  tableState.sortField='id';tableState.sortDirection='asc';
}
function applyDropdowns(d){return d.filter(r=>Object.entries(dropdownState).every(([f,v])=>v==='all'||String(r[f])===String(v)));}
function applyPlots(d,exclude=null){return d.filter(r=>{for(const[source,filters]of Object.entries(plotFilters)){if(source===exclude)continue;for(const[f,v]of Object.entries(filters)){if(f==='ids'){if(!v.includes(r.id))return false;}else if(String(r[f])!==String(v))return false;}}return true;});}
...
function switchTab(key){
  activeTab=key;activeData=configs[key].data;resetState();buildUI();renderDashboard();initializeExportControls();
...
function buildUI(){
  ...
  c.filters.forEach(([f,label])=>{
    dropdownState[f]='all';
    const g=document.createElement('div');g.className='filter-group';
    const l=document.createElement('label');l.htmlFor=`filter-${f}`;l.textContent=label;
    const s=document.createElement('select');s.id=`filter-${f}`;
    const a=document.createElement('option');a.value='all';a.textContent=`All ${label.toLowerCase()}`;s.appendChild(a);
    uniq(activeData,f).forEach(v=>{const o=document.createElement('option');o.value=v;o.textContent=v;s.appendChild(o);});
    s.addEventListener('change',e=>{dropdownState[f]=e.target.value;if(e.target.value==='all')removeFilterFromRecency('dropdown',f);else markFilterAsMostRecent('dropdown',f);renderDashboard();});
    g.append(l,s);fc.appendChild(g);
  });
  [['trend','trendTitle','trendHelp'],['category','categoryTitle','categoryHelp'],['status','statusTitle','statusHelp'],['scatter','scatterTitle','scatterHelp'],['heatmap','heatmapTitle','heatmapHelp'],['distribution','distributionTitle','distributionHelp'],['flow','flowTitle','flowHelp']].forEach(([k,t,h])=>{document.getElementById(t).textContent=c[k].title;document.getElementById(h).textContent=c[k].help;});
  document.getElementById('tableTitle').textContent=`${c.title} records`;
}
function renderDashboard(){
  filteredData=dataFor();renderMetrics();renderActive();renderPlotFilterChips();renderTrend();renderCategory();renderStatus();renderScatter();renderHeatmap();renderDistribution();renderFlow();renderTable();
  setTimeout(attachAllHoverOutlines,0);
}
function renderMetrics(){const m=document.getElementById('metrics');m.innerHTML='';configs[activeTab].metrics.forEach(([l,fn])=>{const c=document.createElement('article');c.className='metric-card';c.innerHTML=`<div class="metric-label">${l}</div><div class="metric-value">${fn(filteredData)}</div>`;m.appendChild(c);});}

function renderActive(){
  const container=document.getElementById('activeFilters');container.innerHTML='';let count=0;
  function addFilterButton({kind,source,field=null,label}){
    const button=document.createElement('button');button.type='button';button.className='active-filter-action';button.setAttribute('aria-label',`Remove filter ${label}`);
    const labelElement=document.createElement('span');labelElement.className='active-filter-action-label';labelElement.textContent=label;
    const removeOverlay=document.createElement('span');removeOverlay.className='active-filter-action-remove';removeOverlay.textContent='Remove filter';
    button.append(labelElement,removeOverlay);button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();clearAppliedFilter({kind,source,field});});container.appendChild(button);count++;
  }
  Object.entries(dropdownState).forEach(([field,value])=>{if(value!=='all')addFilterButton({kind:'dropdown',source:field,field,label:`${prettify(field)}: ${value}`});});
  Object.entries(plotFilters).forEach(([source,filters])=>Object.entries(filters).forEach(([field,value])=>addFilterButton({kind:'plot',source,field,label:`${prettify(source)}: ${field==='ids'?`${value.length} selected`:formatFilterPart(field,value)}`})));
  if(!count){const empty=document.createElement('span');empty.className='no-filter';empty.textContent='No filters applied';container.appendChild(empty);}
}
function formatFilterPart(field,value){if(field==='ids')return `${value.length.toLocaleString()} selected records`;if(field==='department')return `${value}`;return `${prettify(field)}: ${value}`;}
function getFiltersAffectingPlot(){
  const filterMap=new Map();
  Object.entries(dropdownState).forEach(([field,value])=>{if(value==='all')return;const filter={kind:'dropdown',source:field,label:formatFilterPart(field,value)};filterMap.set(filterRecencyKey(filter.kind,filter.source),filter);});
  Object.entries(plotFilters).forEach(([source,sourceFilters])=>{const entries=Object.entries(sourceFilters);if(!entries.length)return;const filter={kind:'plot',source,label:entries.map(([field,value])=>formatFilterPart(field,value)).join(' + ')};filterMap.set(filterRecencyKey(filter.kind,filter.source),filter);});
  const ordered=[];filterRecency.forEach(key=>{if(filterMap.has(key)){ordered.push(filterMap.get(key));filterMap.delete(key);}});filterMap.forEach(filter=>ordered.push(filter));return ordered;
}
function clearAppliedFilter(filter){
  if(filter.kind==='dropdown'){
    dropdownState[filter.source]='all';removeFilterFromRecency('dropdown',filter.source);const select=document.getElementById(`filter-${filter.source}`);if(select)select.value='all';
  }else if(filter.kind==='plot'){
    const sourceFilters=plotFilters[filter.source];
    if(filter.field&&sourceFilters&&Object.prototype.hasOwnProperty.call(sourceFilters,filter.field)){
      delete sourceFilters[filter.field];if(Object.keys(sourceFilters).length===0)removeFilterFromRecency('plot',filter.source);
    }else{plotFilters[filter.source]={};removeFilterFromRecency('plot',filter.source);}
  }
  renderDashboard();
}
function renderPlotFilterChips(){
  document.querySelectorAll('[data-filter-chips]').forEach(container=>{
    const filters=getFiltersAffectingPlot();container.innerHTML='';
    if(!filters.length){const empty=document.createElement('span');empty.className='plot-filter-empty';empty.textContent='No active filters';container.appendChild(empty);return;}
    const stack=document.createElement('div');stack.className='plot-filter-stack';stack.tabIndex=0;stack.setAttribute('aria-label',`${filters.length} active filter${filters.length===1?'':'s'}. Most recent filter is ${filters[0].label}. Hover or focus to expand.`);
    const top=document.createElement('div');top.className='plot-filter-stack-top';top.textContent=filters[0].label;stack.appendChild(top);
    if(filters.length>1){const second=document.createElement('div');second.className='plot-filter-stack-second';second.textContent=filters[1].label;stack.appendChild(second);}
    const menu=document.createElement('div');menu.className='plot-filter-stack-menu';
    filters.forEach(filter=>{const item=document.createElement('button');item.type='button';item.className='plot-filter-stack-item';item.setAttribute('aria-label',`Remove filter ${filter.label}`);const label=document.createElement('span');label.className='plot-filter-stack-item-label';label.textContent=filter.label;const remove=document.createElement('span');remove.className='plot-filter-stack-item-remove';remove.textContent='Remove filter';item.append(label,remove);item.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();clearAppliedFilter(filter);});menu.appendChild(item);});
    stack.appendChild(menu);container.appendChild(stack);
  });
}

function renderTrend(){
  const c=configs[activeTab].trend,d=dataFor('trend'),xs=uniq(d,c.x),bars=xs.map(x=>{const s=d.filter(r=>String(r[c.x])===String(x));return c.valueField?sum(s,c.valueField):s.length;});
  const tr=[{x:xs,y:bars,type:'bar',name:c.valueField||'Count',marker:{color:colors.blue}}];
  if(c.lineNumeric)tr.push({x:xs,y:xs.map(x=>avgRaw(d.filter(r=>String(r[c.x])===String(x)),c.lineNumeric)),type:'scatter',mode:'lines+markers',name:c.lineNumeric,yaxis:'y2',line:{color:colors.green,width:3}});
  else if(c.lineField)tr.push({x:xs,y:xs.map(x=>{const s=d.filter(r=>String(r[c.x])===String(x));return s.length?s.filter(r=>String(r[c.lineField])===String(c.lineValue)).length/s.length*100:0;}),type:'scatter',mode:'lines+markers',name:c.lineField,yaxis:'y2',line:{color:colors.green,width:3}});
  Plotly.react('trendChart',tr,themedLayout({margin:{t:15,r:65,b:55,l:60},xaxis:{title:c.x},yaxis:{title:c.valueField||'Count'},yaxis2:{overlaying:'y',side:'right',ticksuffix:c.lineField&&!c.lineNumeric?'%':''},legend:{orientation:'h',y:1.12}}),plotConfig).then(()=>attachCat('trendChart','trend',c.x,p=>p.x));
}
function renderCategory(){
  const c=configs[activeTab].category,d=dataFor('category'),cats=uniq(d,c.field),stacks=uniq(d,c.stack);
  const tr=stacks.map((sv,i)=>({x:cats,y:cats.map(cat=>{const s=d.filter(r=>String(r[c.field])===String(cat)&&String(r[c.stack])===String(sv));return c.valueField?sum(s,c.valueField):s.length;}),type:'bar',name:sv,marker:{color:[colors.blue,colors.navy,colors.purple,colors.green,colors.orange][i%5]}}));
  Plotly.react('categoryChart',tr,themedLayout({barmode:'stack',margin:{t:15,r:15,b:100,l:55},xaxis:{tickangle:-25},yaxis:{title:c.valueField||'Count'},legend:{orientation:'h',y:1.13}}),plotConfig).then(()=>attachCat('categoryChart','category',c.field,p=>p.x));
}
function renderStatus(){
  const c=configs[activeTab].status,d=dataFor('status'),labels=uniq(d,c.field),values=labels.map(v=>d.filter(r=>String(r[c.field])===String(v)).length);
  Plotly.react('statusChart',[{labels,values,type:'pie',hole:.58,textinfo:'percent'}],themedLayout({margin:{t:20,r:20,b:30,l:20},legend:{orientation:'h',y:-.1}}),plotConfig).then(()=>attachCat('statusChart','status',c.field,p=>p.label));
}
function renderScatter(){
  const c=configs[activeTab].scatter,d=dataFor('scatter'),groups=uniq(d,c.color);
  const tr=groups.map((g,i)=>{const rows=d.filter(r=>String(r[c.color])===String(g));return{x:rows.map(r=>Number(r[c.x])),y:rows.map(r=>Number(r[c.y])),customdata:rows.map(r=>[r.id]),mode:'markers',type:'scattergl',name:g,marker:{size:rows.map(r=>6+Math.min(Math.sqrt(Math.max(Number(r[c.size]||1),1))/8,14)),color:[colors.blue,colors.green,colors.orange,colors.red,colors.purple][i%5],opacity:.65}};});
  Plotly.react('scatterChart',tr,themedLayout({dragmode:'lasso',margin:{t:15,r:30,b:60,l:60},xaxis:{title:c.x},yaxis:{title:c.y},legend:{orientation:'h',y:1.12}}),plotConfig).then(()=>{const p=document.getElementById('scatterChart');clearListeners(p);p.on('plotly_selected',e=>{if(!e?.points?.length)return;markFilterAsMostRecent('plot','scatter');plotFilters.scatter={ids:e.points.map(x=>x.customdata?.[0]).filter(Boolean)};renderDashboard();});});
}
function renderHeatmap(){
  const c=configs[activeTab].heatmap,d=dataFor('heatmap'),rows=uniq(d,c.row),cols=uniq(d,c.col);
  const z=rows.map(rv=>cols.map(cv=>{const s=d.filter(r=>String(r[c.row])===String(rv)&&String(r[c.col])===String(cv));if(!s.length)return null;return c.numeric?avgRaw(s,c.numeric):s.filter(r=>String(r[c.outcome])===String(c.success)).length/s.length*100;}));
  Plotly.react('heatmapChart',[{type:'heatmap',x:cols,y:rows,z,colorscale:[[0,'#d98686'],[.5,'#efcf8c'],[1,'#8abb7c']]}],themedLayout({margin:{t:20,r:70,b:55,l:150},xaxis:{title:c.col}}),plotConfig).then(()=>{const p=document.getElementById('heatmapChart');clearListeners(p);p.on('plotly_click',e=>{const pt=e.points?.[0];if(!pt)return;markFilterAsMostRecent('plot','heatmap');plotFilters.heatmap={[c.row]:pt.y,[c.col]:pt.x};renderDashboard();});});
}
function renderDistribution(){
  const c=configs[activeTab].distribution,d=dataFor('distribution'),cats=uniq(d,c.category);
  const tr=cats.map((cat,i)=>({y:d.filter(r=>String(r[c.category])===String(cat)).map(r=>Number(r[c.value])),type:'box',name:cat,boxpoints:'outliers',marker:{color:[colors.blue,colors.green,colors.orange,colors.purple,colors.navy][i%5]}}));
  Plotly.react('distributionChart',tr,themedLayout({margin:{t:20,r:20,b:100,l:55},yaxis:{title:c.value},xaxis:{tickangle:-25},showlegend:false}),plotConfig).then(()=>{const p=document.getElementById('distributionChart');clearListeners(p);p.on('plotly_click',e=>{const v=e.points?.[0]?.data?.name;if(!v)return;markFilterAsMostRecent('plot','distribution');plotFilters.distribution={[c.category]:v};renderDashboard();});});
}
function renderFlow(){
  const c=configs[activeTab].flow,d=dataFor('flow'),a=uniq(d,c.first),b=uniq(d,c.second),cc=uniq(d,c.third),labels=[...a,...b,...cc],source=[],target=[],value=[];
  a.forEach((x,i)=>b.forEach((y,j)=>{const n=d.filter(r=>String(r[c.first])===String(x)&&String(r[c.second])===String(y)).length;if(n){source.push(i);target.push(a.length+j);value.push(n);}}));
  b.forEach((x,i)=>cc.forEach((y,j)=>{const n=d.filter(r=>String(r[c.second])===String(x)&&String(r[c.third])===String(y)).length;if(n){source.push(a.length+i);target.push(a.length+b.length+j);value.push(n);}}));
  Plotly.react('flowChart',[{type:'sankey',node:{pad:16,thickness:17,label:labels},link:{source,target,value}}],themedLayout({margin:{t:20,r:20,b:20,l:20}}),plotConfig).then(()=>{const p=document.getElementById('flowChart');clearListeners(p);p.on('plotly_click',e=>{const label=e.points?.[0]?.label;if(!label)return;const f=a.includes(label)?c.first:b.includes(label)?c.second:cc.includes(label)?c.third:null;if(!f)return;markFilterAsMostRecent('plot','flow');plotFilters.flow={[f]:label};renderDashboard();});});
}

function exportFilename(source,extension){const dashboardName=(configs[activeTab]?.title||activeTab).toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');return `${dashboardName}-${source}.${extension}`;}
function csvEscape(value){if(value===null||value===undefined)return'';const text=String(value);return /[",\n\r]/.test(text)?`"${text.replace(/"/g,'""')}"`:text;}
function exportPlotCsv(source){const rows=dataFor(source);if(!rows.length)return;const fields=Object.keys(rows[0]),csv=[fields.map(csvEscape).join(','),...rows.map(row=>fields.map(field=>csvEscape(row[field])).join(','))].join('\n'),blob=new Blob([csv],{type:'text/csv;charset=utf-8'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=exportFilename(source,'csv');document.body.appendChild(link);link.click();link.remove();URL.revokeObjectURL(url);}
function getPlotExportTitle(plot){const card=plot.closest('.chart-card'),heading=card?.querySelector('.chart-heading h2, .chart-heading h3');return heading?.textContent?.trim()||'Visualization';}
async function exportPlotImage(plotId,source,format){
  const plot=document.getElementById(plotId);if(!plot)return;const title=getPlotExportTitle(plot),originalTitle=plot.layout?.title?JSON.parse(JSON.stringify(plot.layout.title)):null,originalMargin=plot.layout?.margin?{...plot.layout.margin}:{},exportMargin={...originalMargin,t:Math.max(Number(originalMargin.t||0),70)};
  try{await Plotly.relayout(plot,{title:{text:title,x:.02,xanchor:'left',y:.98,yanchor:'top',font:{size:20}},margin:exportMargin});await Plotly.downloadImage(plot,{format,filename:title.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,''),width:Math.max(plot.clientWidth,900),height:Math.max(plot.clientHeight,550),scale:2});}
  finally{await Plotly.relayout(plot,{margin:originalMargin,title:originalTitle||null});}
}
function closeAllExportMenus(except=null){document.querySelectorAll('.chart-export-control.open').forEach(control=>{if(control!==except){control.classList.remove('open');control.querySelector('.chart-export-button')?.setAttribute('aria-expanded','false');}});}
function initializeExportControls(){
  ...
  Object.entries(plots).forEach(([plotId,source])=>{const plot=document.getElementById(plotId);if(!plot||plot.parentElement.querySelector(`.chart-export-control[data-export-source="${source}"]`))return;const control=document.createElement('div');control.className='chart-export-control';control.dataset.exportSource=source;const button=document.createElement('button');button.type='button';button.className='chart-export-button';button.setAttribute('aria-haspopup','menu');button.setAttribute('aria-expanded','false');button.innerHTML='<span class="chart-export-icon" aria-hidden="true">&#8681;</span><span>Export</span>';const menu=document.createElement('div');menu.className='chart-export-menu';menu.setAttribute('role','menu');[['PNG','png'],['SVG','svg'],['JPEG','jpeg'],['CSV data','csv']].forEach(([label,format])=>{const option=document.createElement('button');option.type='button';option.className='chart-export-option';option.setAttribute('role','menuitem');option.textContent=label;option.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();if(format==='csv')exportPlotCsv(source);else exportPlotImage(plotId,source,format);control.classList.remove('open');button.setAttribute('aria-expanded','false');});menu.appendChild(option);});button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();const willOpen=!control.classList.contains('open');closeAllExportMenus(control);control.classList.toggle('open',willOpen);button.setAttribute('aria-expanded',String(willOpen));});control.append(button,menu);plot.insertAdjacentElement('afterend',control);});
}
...
function attachCat(id,source,field,getter){
  const p=document.getElementById(id);clearListeners(p);
  p.on('plotly_click',e=>{const pt=e.points?.[0];if(!pt)return;markFilterAsMostRecent('plot',source);plotFilters[source]={[field]:getter(pt)};renderDashboard();});
  p.on('plotly_hover',e=>{const pt=e.points?.[0];if(!pt)return;const v=getter(pt);document.getElementById('hoverStatus').textContent=`Highlighting ${field}: ${v}`;highlightTable(field,v);});
  p.on('plotly_unhover',()=>{document.getElementById('hoverStatus').textContent='Hover over a categorical mark to highlight related records. Click it to filter the other views.';highlightTable(null,null);});
}
function clearListeners(p){if(!p?.removeAllListeners)return;['plotly_click','plotly_hover','plotly_unhover','plotly_selected'].forEach(n=>p.removeAllListeners(n));}

function renderTable(){
  const h=document.getElementById('tableHeader'),tb=document.querySelector('#dataTable tbody');h.innerHTML='';tb.innerHTML='';const fields=Object.keys(activeData[0]||{});if(!tableState.sortField||!fields.includes(tableState.sortField))tableState.sortField=fields[0];
  fields.forEach(f=>{const th=document.createElement('th'),b=document.createElement('button');b.className='sort-button';b.type='button';b.textContent=prettify(f);if(tableState.sortField===f){const s=document.createElement('span');s.className='sort-indicator';s.textContent=tableState.sortDirection==='asc'?' ^':' v';b.appendChild(s);}b.addEventListener('click',()=>{if(tableState.sortField===f)tableState.sortDirection=tableState.sortDirection==='asc'?'desc':'asc';else{tableState.sortField=f;tableState.sortDirection='asc';}renderTable();});th.appendChild(b);h.appendChild(th);});
  const sorted=[...filteredData].sort((a,b)=>compare(a[tableState.sortField],b[tableState.sortField],tableState.sortDirection)),shown=sorted.slice(0,250),ranges=numRanges(filteredData,fields);
  shown.forEach(row=>{const tr=document.createElement('tr');tr.dataset.recordId=row.id;fields.forEach(f=>{const td=document.createElement('td');td.textContent=formatVal(f,row[f]);if(ranges[f]&&Number.isFinite(Number(row[f])))td.style.background=numColor(Number(row[f]),ranges[f]);tr.appendChild(td);});tb.appendChild(tr);});
  document.getElementById('tableCount').textContent=`${shown.length.toLocaleString()} shown of ${filteredData.length.toLocaleString()}`;
}
function compare(a,b,d){const f=d==='asc'?1:-1,an=Number(a),bn=Number(b);return Number.isFinite(an)&&Number.isFinite(bn)?(an-bn)*f:String(a??'').localeCompare(String(b??''))*f;}
function prettify(f){return f.replaceAll('_',' ').replace(/\b\w/g,c=>c.toUpperCase());}
function formatVal(f,v){if(v===null||v===undefined)return'';if(f.includes('propensity'))return `${(Number(v)*100).toFixed(1)}%`;if(/salary|budget|actual|amount|capacity|aid_offer|financial_need/.test(f))return `$${Math.round(Number(v)).toLocaleString()}`;if(typeof v==='number'&&!Number.isInteger(v))return v.toFixed(1);return String(v);}
function numRanges(d,fields){const o={};fields.forEach(f=>{const v=d.map(r=>Number(r[f])).filter(Number.isFinite);if(v.length)o[f]={min:Math.min(...v),max:Math.max(...v)};});return o;}
function numColor(v,r){const t=clamp((v-r.min)/((r.max-r.min)||1),0,1),h=210-t*105;return getTheme()==='dark'?`hsl(${h} 38% ${18+t*7}%)`:`hsl(${h} 55% ${96-t*14}%)`;}
function highlightTable(f,v){document.querySelectorAll('#dataTable tbody tr').forEach(row=>{if(!f){row.style.opacity='1';return;}const rec=filteredData.find(x=>x.id===row.dataset.recordId);if(rec)row.style.opacity=String(rec[f])===String(v)?'1':'.22';});}

document.querySelectorAll('.tab-button').forEach(b=>b.addEventListener('click',()=>switchTab(b.dataset.tab)));
document.getElementById('resetAll').addEventListener('click',()=>{resetState();buildUI();renderDashboard();});
initializeFilterAnchorControl();
initTheme();
switchTab('student');
