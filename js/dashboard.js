...
function initTheme(){const saved=localStorage.getItem('analytics-theme-mode');themeState.mode=['system','light','dark'].includes(saved)?saved:'system';const media=window.matchMedia('(prefers-color-scheme: dark)');const apply=()=>{document.documentElement.setAttribute('data-theme',resolvedTheme());updateThemeButton();if(activeData.length)renderDashboard()};document.getElementById('themeToggle').addEventListener('click',()=>{const order=['system','light','dark'];themeState.mode=order[(order.indexOf(themeState.mode)+1)%order.length];localStorage.setItem('analytics-theme-mode',themeState.mode);apply()});media.addEventListener('change',()=>{if(themeState.mode==='system')apply()});apply()}function updateThemeButton(){const b=document.getElementById('themeToggle'),i=document.getElementById('themeIcon'),l=document.getElementById('themeLabel');if(themeState.mode==='system'){i.textContent='◐';l.textContent='System'}else if(themeState.mode==='light'){i.textContent='☀';l.textContent='Light'}else{i.textContent='☾';l.textContent='Dark'}b.setAttribute('aria-label',`Change appearance. Current setting: ${l.textContent}`);b.title=`Appearance: ${l.textContent}. Click to cycle System, Light, and Dark.`}
...
function renderResets(){document.querySelectorAll('.plot-reset').forEach(b=>b.disabled=!Object.keys(plotFilters[b.dataset.reset]||{}).length)}
